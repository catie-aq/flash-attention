...
def test_bias_attention(bs_seqlen, nh_headdim, dtype):
    ...
    q, k, v, _, _ = get_tensors(bs_seqlen[0], bs_seqlen[1], nh_headdim[0], nh_headdim[1], dtype)
    dout = torch.rand_like(q)

    slopes = torch.Tensor(get_slopes(nh_headdim[0])).cuda()
    context_position = torch.arange(bs_seqlen[1])[:, None].cuda()
    memory_position = torch.arange(bs_seqlen[1])[None, :].cuda()
    relative_position = memory_position - context_position
    relative_position = torch.abs(relative_position).unsqueeze(0).expand(nh_headdim[0], -1,-1)
    symetric_alibi = ((slopes*-1).unsqueeze(1).unsqueeze(1)) * relative_position
    symetric_alibi = symetric_alibi.view(1, nh_headdim[0], bs_seqlen[1], bs_seqlen[1])
    ...
    alibi_ratio = (2.0 ** (-2.0 ** -(math.log2(nh_headdim[0]) - 3)))
    alibi_start = alibi_ratio
